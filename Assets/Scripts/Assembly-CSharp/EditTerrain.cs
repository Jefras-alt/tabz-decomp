...
public class EditTerrain
{
	...
	public static Terrain GetTerrain(Vector3 worldPos)
	{
		...
		for (int i = 0; i < current.terrainAreas[0].terrains.Count; i++)
		{
			TCUnityTerrain tCUnityTerrain = current.terrainAreas[0].terrains[i];
			if (!(tCUnityTerrain.terrain == null) && !(tCUnityTerrain.terrain.terrainData == null))
			{
				Rect rect = new Rect(tCUnityTerrain.terrain.transform.position.x, tCUnityTerrain.terrain.transform.position.z, tCUnityTerrain.terrain.terrainData.size.x, tCUnityTerrain.terrain.terrainData.size.z);
				if (rect.Contains(worldPos))
				{
					return tCUnityTerrain.terrain;
				}
			}
		}
		return null;
