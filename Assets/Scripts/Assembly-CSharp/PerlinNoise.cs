...
public class PerlinNoise
{
	...
	private void LoadPermTable1D()
	{
		if (!m_permTable1D)
		{
			m_permTable1D = new Texture2D(256, 1, TextureFormat.Alpha8, false, true);
	...
	private void LoadPermTable2D()
	{
		...
		m_permTable2D = new Texture2D(256, 256, TextureFormat.ARGB32, false, true);
	...
	private void LoadGradient3D()
	{
		if (!m_gradient3D)
		{
			m_gradient3D = new Texture2D(256, 1, TextureFormat.RGB24, false, true);
	...
	private void LoadGradient4D()
	{
		if (!m_gradient4D)
		{
			m_gradient4D = new Texture2D(256, 1, TextureFormat.ARGB32, false, true);
